...
# TODO: generate additional plots dynamically in RShiny
# only vars needed (resObject, geneList) are saved as RDS

plot_results<- function(resObject, geneList, db.name, methodType){
  p <- dotplot(resObject, 
               showCategory = 20,
               label_format=50) 
  dotgr.fn <- paste(ds.noext, db.name, methodType, "dot_gr.pdf", sep = "_")
  ggsave(p, file = file.path(output.dir, methodType,"plots",dotgr.fn), 
  ...
  ggsave(p, file = file.path(output.dir, methodType,"plots",dotcnt.fn), 
         width = 2400, height = 2400, units = "px", device='pdf')
  
  p <- cnetplot(resObject, foldChange=geneList,
                categorySize="geneNum", 
                cex_label_category = 0.8, 
                cex_label_gene = 1.0)
  cnet1.fn <- paste(ds.noext, db.name, methodType,"cnet1.pdf", sep = "_")
  ggsave(p, file = file.path(output.dir, methodType,"plots",cnet1.fn), 
         width = 2400, height = 2400, units = "px", device='pdf')
  ...
  ggsave(p, file = file.path(output.dir, methodType,"plots",cnet2.fn), 
         width = 2400, height = 2400, units = "px", device='pdf')
  ...
  ggsave(p, file = file.path(output.dir, methodType,"plots",emap.fn), 
         width = 2400, height = 2400, units = "px", device='pdf')
  ...
  if(methodType == "ora"){
    ...
    ggsave(p, file = file.path(output.dir, methodType,"plots",upset.fn), 
